...
import { useState, useEffect } from "react"; // Agregamos hooks para control de montaje
import { MessageSquare } from "lucide-react";

export default function WhatsAppButton() {
  const [hasMounted, setHasMounted] = useState(false);

  // 1. Solo permitimos que el botón exista después de que la app cargó en el navegador
  useEffect(() => {
    setHasMounted(true);
  }, []);
  ...
  // 2. Si todavía estamos en el servidor (o cargando), no renderizamos nada para evitar el error de hidratación
